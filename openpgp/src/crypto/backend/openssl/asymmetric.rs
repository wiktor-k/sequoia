...
impl TryFrom<&ProtectedMPI> for BigNum {
    ...
    fn try_from(mpi: &ProtectedMPI) -> std::result::Result<BigNum, crate::Error> {
        BigNum::from_slice(mpi.value()).map_err(|_| crate::Error::InvalidArgument("bignum".into()))
    }
}

impl TryFrom<&MPI> for BigNum {
    type Error = crate::Error;
    fn try_from(mpi: &MPI) -> std::result::Result<BigNum, crate::Error> {
        BigNum::from_slice(mpi.value()).map_err(|_| crate::Error::InvalidArgument("bignum".into()))
    }
}
        ...
        self.secret().map(
            |secret| match (self.public().pk_algo(), self.public().mpis(), secret) {
                ...
                | (
                    ...
                    ctx.sign_init()?;
                    let len = ctx.sign_to_vec(&v, &mut signature)?;

                    Ok(mpi::Signature::RSA {
                        s: mpi::MPI::new(&signature[..len]),
                    })
                }
                ...
                (
                    ...
                    Ok(mpi::Signature::ECDSA {
                        r: MPI::new(&sig.r().to_vec()),
                        s: MPI::new(&sig.s().to_vec()),
                    })
                }
    ...
    R: key::KeyRole,
{
        ...
        T: Into<Option<SystemTime>>,
    {
        ...
        let mut private_key = key.raw_private_key()?;
        private_key.reverse();
        ...
        T: Into<Option<SystemTime>>,
    {
        ...
        let big_p = BigNum::from_slice(p)?;
        let big_q = BigNum::from_slice(q)?;
        let n = &big_p * &big_q;
        ...
        e.mod_inverse(&d, &big_phi, &mut ctx)?; // e ≡ d⁻¹ (mod 𝜙)
    ...
    pub fn generate_ecc(for_signing: bool, curve: Curve) -> Result<Self> {
        ...
        if curve == Curve::Cv25519 || curve == Curve::Ed25519 {
            ...
            let mut scalar = key.raw_private_key()?;
